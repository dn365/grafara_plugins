...
function (angular, _, MonQueryBuilder) {
  'use strict';

  var module = angular.module('grafana.controllers');

  module.controller('MonQueryCtrl', function($scope, $timeout, $sce, templateSrv, $q) {
    ...
    $scope.init = function() {
      ...
      $scope.queryBuilder = new MonQueryBuilder(target);
    ...
    $scope.getTagsOrValues = function(segment, index) {
      ...
      if (segment.type === 'key' || segment.type === 'plus-button') {
        ...
        return $q.when([new MetricSegment('AND')]);
      } else if (segment.type === 'operator' && /^(?!\/.*\/$)/.test($scope.tagSegments[index+1].value)) {
        return $q.when([MetricSegment.newOperator('=')]);
      ...
      // console.log("QueryCtrl getTagsOrValues: --------");
      // console.log(segment);
      // console.log(query);
      return $scope.datasource.metricFindQuery(query)
      .then($scope.transformToSegments)
    ...
    $scope.addField = function() {
      $scope.target.fields.push({name: $scope.addFieldSegment.value, func: 'avg'});
