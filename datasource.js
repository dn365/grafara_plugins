...
function (angular, _, $, config, kbn, moment) {
  ...
  module.factory('MonDatasource', function($q, backendSrv, templateSrv) {
    ...
    MonDatasource.prototype.convertDataPointsToMs = function(result) {
    ...
    MonDatasource.prototype.events = function(options) {
      try {
        var tags = '';
        if (options.tags) {
    ...
    MonDatasource.prototype.translateTime = function(date, rounding) {
      if (_.isString(date)) {
        if (date === 'now') {
          return 'now';
        }
    ...
    MonDatasource.prototype.loadDashboard = function(dashName) {
      return this.doGraphiteRequest({method: 'GET', url: '/dashboard/load/' + encodeURIComponent(dashName) });
    };

    MonDatasource.prototype.doGraphiteRequest = function(options) {
    ...
    MonDatasource.prototype._seriesRefLetters = [
      '#A', '#B', '#C', '#D',
      '#E', '#F', '#G', '#H',
      '#I', '#J', '#K', '#L',
      '#M', '#N', '#O', '#P',
    ...
    MonDatasource.prototype.buildGraphiteParams = function(options) {
